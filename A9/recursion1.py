...
def array220(nums, index):
   if index > (len(nums) - 2):
      ...
      return True
   elif (nums[index + 1] % (nums[index] * 10) ) == 0:
      return True
   else:
...
def countHi2(str):
   if len(str) < 2: 
      ...
      if str[(len(str)-2):(len(str))] == 'hi':
         ...
         return 0
   elif str[(len(str)-2):(len(str))] == 'hi' and str[(len(str)-3)] != 'x':
      return 1 + countHi2(str[:(len(str)-2)])
   else:
...
def nestParen(str):
   if len(str) == 0:
      ...
      if str[len(str)-1] == ')':
         return True and nestParen(str[1:len(str)-1])
      else:
         return False
   else:
      return False
