...
#  Description: A series of recursion functions designed to further understand the concept
...
def groupSumClump(start, nums, target):
  ...
  if start >= len(nums):
    ...
    if nums[start] == nums[comparison]:
      while nums[start] == nums[comparison]:
        comparison += 1
      adjCount = comparison - start
      return groupSumClump(comparison, nums, target - (adjCount * nums[start])) \
      or groupSumClump(comparison, nums, target)
    else:
      return groupSumClump(start+1, nums, target - nums[start]) \
      or groupSumClump(start+1, nums, target)
  else:
    return groupSumClump(start+1, nums, target - nums[start]) \
    or groupSumClump(start+1, nums, target)
...
def splitArrayHelper(start, nums, sum1, sum2):
  if start >= len(nums):
    ...
    val = nums[start]
    return splitArrayHelper(start+1, nums, sum1+val, sum2) \
    or splitArrayHelper(start+1, nums, sum1, sum2+val)
...
def splitOdd10Helper(start, nums, sum1, sum2):
  if start >= len(nums):
    return (((sum1 % 10) == 0) and ((sum2 % 2) != 0)) or \
    (((sum2 % 10) == 0) and ((sum1 % 2) != 0))
  else:
    val = nums[start]
    return splitOdd10Helper(start+1, nums, sum1+val, sum2) \
    or splitOdd10Helper(start+1, nums, sum1, sum2+val)
...
def split53Helper(start, nums, sum1, sum2):
  if start >= len(nums):
    ...
    val = nums[start]
    if val % 5 == 0:
      return split53Helper(start+1, nums, sum1+val, sum2)
    elif val % 3 == 0 and val % 5 != 0:
      return split53Helper(start+1, nums, sum1, sum2+val)
    else:
      return split53Helper(start+1, nums, sum1, sum2+val) \
      or split53Helper(start+1, nums, sum1+val, sum2)
