...
def insert_word (s, hash_table):
  ...
  if not hash_table[hash_idx]:
    hash_table[hash_idx] = s
  else:
    ss = step_size(s, const)
    while True:
      hash_idx += ss
      if not hash_table[hash_idx]:
        hash_table[hash_idx] = s
...
def is_reducible (s, hash_table, hash_memo):
  ...
  if s in hash_memo or s == 'a' or s == 'i' or s == 'o':
    return True
  else:
...
def get_longest_words (string_list):
  ...
  for i in range(len(string_list)):
    if len(string_list[i]) == length:
      max_length.append(string_list[i])
  return max_length
...
def main():
  ...
  # find the largest reducible words in reducible_words
    max_len = len(max(reductibles, key=len))
    
    # I'm p sure that Its already in alpha order, but safety measure
    # reductibles.sort()
    
    for word in reductibles:
      if len(word) == max_len:
        print(word)
